...
choice::AddressBook address_book;
choice::AddressBook raddress_book;
...
void PromptForAddress(choice::Person* person) {
	sleep(1);
	cin.clear();
	cout << "Inside PromptForAddress" << endl;	
  	...
  	cout << "Exiting PromptForAddress" << endl;
}
...
int main(int argc, char *argv[]) {
	...
	while(true) {
		...
		if (output.size() > 0) {
			...
			n = write(comm_socket, output.c_str(), output.size() + 1);
...
void *async_read_loop(void *arg) {
	...
	while(!connected);
	cout << ">>>> Connection established with client on " << comm_addr_str << " " << comm_port << endl;
	...
	while(true) {
		...
		if (activity >= 0 && FD_ISSET(comm_socket, &master)) {
			bzero(buffer, 4096);
			n = read(comm_socket, buffer, 4096);
			if (n < 0) error("ERROR reading from socket\n");
			raddress_book.ParseFromString(buffer);
			for (int i = 0; i < raddress_book.people_size(); i++) {
				const choice::Person& person = raddress_book.people(i);
